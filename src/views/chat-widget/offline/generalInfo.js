...
const GeneralInfo = (props) => {
  ...
  useEffect(async () => {
    await axios
      .get(
        `${BASE_URL}/api/conversation-management/chat-widget/select/offline-chat-button/${id}`
      )
      .then((response) => {
  ...
  const onSubmit = async () => {
    if (1) {
      ...
      await axios
        .post(
          `${BASE_URL}/api/conversation-management/chat-widget/edit/offline-chat-button/${id}`,
          {
