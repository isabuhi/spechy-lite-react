...
const GeneralInfo = (props) => {
  ...
  useEffect(async () => {
    await axios
      .get(`${BASE_URL}/api/project-management/reason-codes/ticket/select/${id}`)
      .then((response) => {
  ...
  useEffect(async () => {
    ...
    //---------
    await axios
    .get(`${BASE_URL}/api/project-management/reason-codes/ticket/status-types`)
    .then((response) => {
      if (response.status === 200) {
        let statusTemp = [];
        let result = response.data.data;
  ...
  const onSubmit = async () => {
    if (1) {
      await axios
      .post(`${BASE_URL}/api/project-management/reason-codes/ticket/update/${id}`, {
        ...
        <Form onSubmit={handleSubmit(onSubmit)}>
          <Row>
            <Col md={{ size: 6, offset: 2 }}>
              <FormGroup>
                <Label for="attached">
                  <FormattedMessage id="AttachedSource"></FormattedMessage> :  <span className="text-danger">*</span>
                </Label>
                <DualList projectList={projects} projectIds={projectIds}/>
              </FormGroup>
              <FormGroup>
                <Label for="reasonName">
                  <FormattedMessage id="ReasonName"></FormattedMessage> : <span className="text-danger">*</span>
                </Label>
                <Input
                  type="text"
                  id="reasonName"
              ...
              <FormGroup>
                <Label for="call">
                  <FormattedMessage id="Status"></FormattedMessage> :  <span className="text-danger">*</span>
                </Label>
                <Select
              ...
              <FormGroup>
                <Label for="type">
                  <FormattedMessage id="ReasonCodeType"></FormattedMessage> :  <span className="text-danger">*</span>
                </Label>
                <Select
                  theme={selectThemeColors}
                  className='react-select'
              ...
              <FormGroup>
                <Label for="sms">
                  <FormattedMessage id="SelectSMSTemplate"></FormattedMessage> :  <span className="text-danger">*</span>
                </Label>
                <Select
                  theme={selectThemeColors}
                  className='react-select'
