...
class WorkingHours extends React.Component {
  ...
  updateSelectInternal(state) {
    ...
    const finalData = [];
    for (let d = 0; d < this.state.days.length; d += 1) {
      const timesData = this.state.days[d].timeCells;
      const ranges = getWorkingHoursSelectedRanges(timesData);

      const dayData = this.props.days[d];

      _.each(ranges, (r, index) => {
        const from = timesData[r.start].timeFrom;

        const to = timesData[r.end].timeTo;

        finalData.push([dayData.name, [from, to]]);
      });
    }
    this.props.getTheHours(finalData);
    this.setState({
      hoursAndDays: finalData,
    });
  }
  ...
  render() {
    ...
    const timeHeaders = _.chain(_.range(24))
      .map((i) => [
        <td key={i} className="header1">
          <span>{i < 10 ? `0${i}` : i}</span>
        </td>,
        <td key={`${i}-part`} className="header1 part">
          <span>30</span>
        </td>,
      ])
      .flatten()
    ...
    for (let d = 0; d < this.state.days.length; d += 1) {
      ...
      _.each(ranges, (r, index) => {
        ...
        // console.log("facaceac", from, to, day.name);
