...
// ** Store & Actions
import { getChatVedioList } from "../store/actions";
...
const DataTableServerSide = () => {
  ...
  const handleAddClick = async () => {
    await axios
      .get(`${BASE_URL}/api/conversation-management/video-chat-widget/create`)
      .then((response) => {
  ...
  const store = useSelector((state) => state.chatWidgetVedio);
  // ** States
    ...
    <Fragment>
      ...
      <Card>
        <CardHeader className="border-bottom">
          <CardTitle tag="h4">Video Chat Widget List</CardTitle>
          <Button className="ml-2" color="primary" onClick={handleAddClick}>
            <Plus size={15} />
            <span className="align-middle ml-50">Add Video Chat Widget</span>
          </Button>
        </CardHeader>
