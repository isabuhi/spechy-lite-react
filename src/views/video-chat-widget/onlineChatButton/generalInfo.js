...
const GeneralInfo = (props) => {
  ...
  useEffect(async () => {
    await axios
      .get(
        `${BASE_URL}/api/conversation-management/video-chat-widget/select/online-chat-button/${id}`
      )
      .then((response) => {
        if (response.status === 200) {
          ...
          setIconOne(response.data.data.icon_color);
          ...
          if (response.status === 200) {
            toast.success(
              <ToastContent
                type={"success"}
                content={
                  "You have successfully update chat widget online chat button."
                }
