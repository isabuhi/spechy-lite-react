...
// ** Store & Actions
import { getFaceList } from "../store/actions";
...
const DataTableServerSide = () => {
  ...
  const setPagePermissions = async (response) => {
    ...
    await axios
      .post(
        `${BASE_URL}/api/facebook-management/account/set-page-permissions`,
        {
      ...
      .then((res) => {
        console.log("resofpersmissions", res);
        if (res.status === 200) {
          toast.success(
            <ToastContent
              type={"success"}
    ...
    <Fragment>
      <Card>
        <CardHeader className="border-bottom">
          <CardTitle tag="h4">Facebook Accounts</CardTitle>
