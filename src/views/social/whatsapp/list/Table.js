...
// ** Store & Actions
import { getWhatsappList, deleteWhatsapp } from "../store/actions";
...
const DataTableServerSide = () => {
  ...
  const handleAddClick = () => {
    history.push("/social/whatsapp/add");
  };
  // ** Store Vars
  ...
  const dataToRender = () => {
    ...
    /*if (store.templatesList.length > 0) {
      return store.templatesList;
    } else if (store.templatesList.length === 0 && isFiltered) {
      return [];
    } else {
  ...
  return (
    <Fragment>
      ...
      >
        ...
        <ModalBody>Are you sure to delete this whatsapp account?</ModalBody>
        <ModalFooter>
          <Button.Ripple
            color="danger"
            onClick={() =>
              dispatch(
                deleteWhatsapp(store.isDeleteModalOpen.id),
      ...
      <Card>
        <CardHeader className="border-bottom">
          <CardTitle tag="h4">Whatsapp Accounts</CardTitle>
          <Button className="ml-2" color="primary" onClick={handleAddClick}>
            <Plus size={15} />
            <span className="align-middle ml-50">Add Whatsapp</span>
          </Button>
        </CardHeader>
